...
BEGIN_EVENT_TABLE(AddOrDeleteParametersDialog, wxDialog)
    EVT_BUTTON(ID::AddParameter,AddOrDeleteParametersDialog::OnAddParameter)
    EVT_BUTTON(ID::DeleteParameter,AddOrDeleteParametersDialog::OnDeleteParameter)
END_EVENT_TABLE()

AddOrDeleteParametersDialog::AddOrDeleteParametersDialog(wxWindow* parent,BaseRD* system)
    : wxDialog(parent,wxID_ANY,_("Add or delete parameters"),wxDefaultPosition,wxDefaultSize,wxDEFAULT_DIALOG_STYLE|wxRESIZE_BORDER)
{
    ...
    wxBoxSizer *hsizer = new wxBoxSizer(wxHORIZONTAL);
    {
        ...
        hsizer->Add(list,wxSizerFlags(1).Border().Expand());
        for(int i=0;i<system->GetNumberOfParameters();i++)
            list->Append(system->GetParameterName(i));
    }
    {
        wxBoxSizer* vsizer2 = new wxBoxSizer(wxVERTICAL);
        vsizer2->Add(new wxButton(this,ID::AddParameter,_("Add")));
        vsizer2->Add(new wxButton(this,ID::DeleteParameter,_("Delete")));
        hsizer->Add(vsizer2);
    }
    vsizer->Add(hsizer,wxSizerFlags(1).Expand());

    vsizer->Add(CreateSeparatedButtonSizer(wxOK),wxSizerFlags().Border().Expand());

    this->SetSizerAndFit(vsizer);
}

void AddOrDeleteParametersDialog::OnAddParameter(wxCommandEvent& event)
{
    wxMessageBox(_("TODO!!!"));
}

void AddOrDeleteParametersDialog::OnDeleteParameter(wxCommandEvent& event)
{
    wxMessageBox(_("TODO!!!"));
}
