...
'Rdy Houdini' is a set of Houdini Digital Assets ('.hdanc' files, the non-commercial variant) that import Ready VTI files into Houdini as a 2D Volume/voxel-based Dop object and formula source for Gas OpenCL microsolver Dops. It mostly imports VTI's that use Formula mode, and a couple that use Full Kernel mode have worked as well (so far, more supported Ready formula types TBA!).

The hdanc files will ship with Ready 0.9, and to import new patterns they rely on some new revisions to the 'rdy' command binary which are also shipping in 0,9. This is Beta software, and it is expected that there will be some teething issues with it working flawlessly on all platforms. The platform that has been tested most is Linux. The version of Houdini it was made and tested with is 17.5 and it is untested in 18.0 as of yet.
...
The Houdini parameter UI can be very sluggish if you try to view the initial-state parms (on the Null rd_object/GET_vti_info), as that node can have some *very* long string values on its parameters. This is particularly an issue for large simulations (1024+ square voxels or more). Don't try to view the parameter interface of that null unless you just want to see how slow it is.

Only 2D simulations are supported so far, but it won't be hard to extend it to 3D eventually. Potentially the runs-on-a-polymesh type of Ready-scene may be able to be supported too (and we can still stay using OpenCL). Please contact Dan Wills (email below) to co-ordinate if you want to contribute to any of these efforts.
...
Ready setups will load much quicker if you first save them with small, (for example 256x256-voxel) initial states. You can still run the sims at a higher resolution once they are imported to Houdini by using the resolution override parameters. Settings are provided (on the 'RD Object' node) that you can use to perturb the initial states with Improved Perlin Noise. If you are increasing the resolution, the initial state will tile out (ie it will repeat) but can be made nonperiodic using some noises. The noises presently only affect the first 2 reagents, but since this is Houdini, the setup could easily be extended to initialise any number of reagents in any way that you please.
...
The main code that drives the Rdy Houdini HDAs is contained in the 'HDA Module' sections of their type properties. To see or modify the code, check out the Scripts tab. (On the node's right-click menu, choose 'Type Properties..' then go to the 'Scripts' Tab). If you find that you want to change anything, you can unlock the HDA definition ("Allow Editing of Contents", and edit the code, press apply, and then save (and ideally lock: "Match Current Definition") the HDA. 
...
If you are having troubles loading new VTIs you should check that the path to the 'rdy' binary is correctly specified on the 'Ready Install Path' parameter on the rd_object Dop node. Please note that this is beta software and might not work for everyone yet.
