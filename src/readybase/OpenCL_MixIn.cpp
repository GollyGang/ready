...
//Probably should wrap this in if defined(__EXTERNAL_OPENCL__).
OpenCL_MixIn::OpenCL_MixIn( int opencl_platform, int opencl_device, cl_context externalContext )
{
    ...
    if(LinkOpenCL()!= CL_SUCCESS)
        throw runtime_error("Failed to link external context for OpenCL");
...
void OpenCL_MixIn::TestKernel(std::string kernel_source)
{
    ...
    // build the program
    ret = clBuildProgram(temp_program,1,&this->device_id,"",NULL,NULL);
    if(ret != CL_SUCCESS)
    {
