...
//
// TODO: Make the plugin able to create its own volume primitives (rather than rely on readyVolume.otl).
//       Add arbitrary amount of output reagents (instead of just 3).
//       Sort out something better for the parameter interface, stop regenerating it.. use a MultiParm.
//       Work out how to get data back and forth to the running system, to allow feedback between Houdini and Ready simulations.
//       See if some kind of handle to the shader can be got so that parameter values can be updated live without recompilation.
//       Get it working with meshes too, and ones supplied from inside houdini.
//
// DONE: Turn off almost all the prints (cout calls) - DONE
//       Remove 'Mask' stuff from everywhere until it works - DONE

// Here's one hcustom line that worked:
// hcustom readyHoudini.C -g -lOpenCL -I../../readybase -I/usr/include/vtk-6.1 -L~/dev/vtk/vtk.git/lib -L../../../ -lreadybase -lvtkCommonCore -lvtkIOXML -lvtkInteractionStyle -lvtkRenderingFreeType -lvtkRenderingOpenGL
...
{
    // Previously: code duplication here from frame.cpp,
	// Solution: SetDefaultRenderSettings method in Properties class that both can call.
...
void newSopOperator(OP_OperatorTable *table)
{
     table->addOperator(new OP_Operator("ready_rd",
					"Ready RD",
					 ...
					 0));
	 // cout << "Done.\n";
	 cout << "Defined ready_rd compiled operator type . . .\n";
}
...
//	PRM_Name("applyMasks","Apply Masks"),
//	PRM_Name("maskMultR","Mask Mult R"),
//	PRM_Name("maskMultG","Mask Mult G"),
//	PRM_Name("maskMultB","Mask Mult B"),
//	PRM_Name("maskAddR","Mask Add R"),
//	PRM_Name("maskAddG","Mask Add G"),
//	PRM_Name("maskAddB","Mask Add B"),
};
...
static PRM_Default parm_defaults[] =
{
	...
	PRM_Default( 0 ), //reagentR
	PRM_Default( 0 ), //reagentG
	PRM_Default( 0 ), //reagentB
	PRM_Default( 256 ), //xres
    PRM_Default( 256 ), //yres
    PRM_Default( 1 ), //zres
    PRM_Default( 0 ), //regenerateInitialState
//	PRM_Default( 0 ), //applyMasks
//	PRM_Default( 0 ), //maskMultR
//	PRM_Default( 0 ), //maskMultG
//	PRM_Default( 0 ), //maskMultB
//	PRM_Default( 0 ), //maskAddR
//	PRM_Default( 0 ), //maskAddG
//	PRM_Default( 0 ), //maskAddB
};
...
{
    PRM_Template(parm_types[0], 1, &parm_names[0], &parm_defaults[0]), //vtiFile
    PRM_Template(parm_types[1], 1, &parm_names[1], &parm_defaults[1]), //writeAttribute
    PRM_Template(parm_types[2], 1, &parm_names[2], &parm_defaults[2]), //startFrame
    PRM_Template(parm_types[3], 1, &parm_names[3], &parm_defaults[3]), //stepsPerFrame
    PRM_Template(parm_types[4], 1, &parm_names[4], &parm_defaults[4]), //reagentR
    PRM_Template(parm_types[5], 1, &parm_names[5], &parm_defaults[5]), //reagentG
    PRM_Template(parm_types[6], 1, &parm_names[6], &parm_defaults[6]), //reagentB
    PRM_Template(parm_types[7], 1, &parm_names[7], &parm_defaults[7]), //xRes
    PRM_Template(parm_types[8], 1, &parm_names[8], &parm_defaults[8]), //yRes
    PRM_Template(parm_types[9], 1, &parm_names[9], &parm_defaults[9]), //zRes
    PRM_Template(parm_types[10], 1, &parm_names[10], &parm_defaults[10]), //regenerateInitialState
    PRM_Template() // sentinel
//    PRM_Template(parm_types[7], 1, &parm_names[7], &parm_defaults[7]), //applyMasks
//    PRM_Template(parm_types[8], 1, &parm_names[8], &parm_defaults[8]), //multR
//    PRM_Template(parm_types[9], 1, &parm_names[9], &parm_defaults[9]), //multG
//    PRM_Template(parm_types[10], 1, &parm_names[10], &parm_defaults[10]), //multB
//    PRM_Template(parm_types[11], 1, &parm_names[11], &parm_defaults[11]), //addR
//    PRM_Template(parm_types[12], 1, &parm_names[12], &parm_defaults[12]), //addG
//    PRM_Template(parm_types[13], 1, &parm_names[13], &parm_defaults[13]), //addB
};
...
void readyHoudini::updateVtiFile( const char *update_file )
{
	...
	// cl_kernel test = (OpenCLImageRD)(this->system)->GetClKernel();
	//OpenCLImageRD* test =  (OpenCLImageRD*)(this->system);
	//cl_kernel test_kernel = test->GetClKernel();
	//cl_uint num_args;
	//size_t ret_size;
	//clGetKernelInfo( test_kernel, CL_KERNEL_NUM_ARGS, sizeof(cl_uint), &num_args, &ret_size );
	// cout << "Number of kernel args is: " << num_args << "\n";
...
void readyHoudini::updateSystemFromParms( OP_Context &context )
{
	...
	if ( readyHoudini::parmChanged( context ) )
	{
        ...
        // REGEN()  (defined in readyHoudini.h) checks the value of a particular parameter called regenerateInitialState.
        int regen = REGEN();
        if (regen)
        {
